...
const StatCard = ({ label, value, color }) => (
  <Paper sx={{ p: 2, textAlign: "center", bgcolor: "#ffffff", borderRadius: 2 }}>
...
const BacktestView = () => {
  const { data: strategies } = useResource("backtest-strategies", "/backtest/strategies/");
  ...
  }, [selectedStrategy, selectedSector, startDate, endDate, initialCash, params]);
      ...
      <Container maxWidth={false}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} md={3}>
            <Paper sx={{ p: 2, bgcolor: "#ffffff", borderRadius: 2 }}>
              ...
              <FormControl fullWidth size="small" sx={{ mb: 2 }}>
                ...
                <Select value={selectedStrategy} onChange={handleStrategyChange} label="Strategy">
                  {(strategies || []).map((s) => (
          ...
          <Grid item xs={12} md={9}>
            {loading && (
              <Paper sx={{ p: 3, bgcolor: "#ffffff", borderRadius: 2, textAlign: "center" }}>
                <Typography variant="h6" gutterBottom>
                ...
                <Typography variant="caption" color="text.secondary" mt={1}>
                  {progress > 0 ? `Processing combination ${Math.round(progress * 0.3)}/30...` : "Queued, waiting for worker..."}
                </Typography>
              </Paper>
            )}
            ...
            {results && !loading && (
              <Stack spacing={2}>
                ...
                <Grid container spacing={1}>
                  <Grid item xs={6} sm={3}>
                    <StatCard
                      ...
                      color={results.total_return_pct > 0 ? "#10b981" : "#ef4444"}
                    />
                  </Grid>
                  <Grid item xs={6} sm={3}>
                    <StatCard
                  ...
                  <Grid item xs={6} sm={3}>
                    <StatCard label="Avg Hold" value={`${results.avg_hold_days}d`} />
                  </Grid>
                  <Grid item xs={6} sm={3}>
                ...
                <Paper sx={{ bgcolor: "#1e293b", borderRadius: 2 }}>
                  <Typography variant="subtitle2" color="text.secondary" p={2} pb={0}>
                  ...
                  <TableContainer sx={{ maxHeight: 300 }}>
                    <Table size="small" stickyHeader>
                      <TableHead>
                        <TableRow>
                          <TableCell sx={{ bgcolor: "#334155", color: "#f8fafc" }}>Date</TableCell>
                          <TableCell sx={{ bgcolor: "#334155", color: "#f8fafc" }}>Symbol</TableCell>
                          <TableCell sx={{ bgcolor: "#334155", color: "#f8fafc" }}>Action</TableCell>
                          <TableCell sx={{ bgcolor: "#334155", color: "#f8fafc" }} align="right">Price</TableCell>
                          <TableCell sx={{ bgcolor: "#334155", color: "#f8fafc" }} align="right">Shares</TableCell>
                          <TableCell sx={{ bgcolor: "#334155", color: "#f8fafc" }} align="right">P&L</TableCell>
                        </TableRow>
                      </TableHead>
                      <TableBody>
                        {results.trades.slice(0, 100).map((t, i) => (
                          <TableRow key={i} sx={{ "&:hover": { bgcolor: "#334155" } }}>
                            <TableCell sx={{ color: "#e2e8f0" }}>{t.date}</TableCell>
                            <TableCell>
                            ...
                            <TableCell align="right" sx={{ color: "#e2e8f0" }}>${t.price}</TableCell>
                            <TableCell align="right" sx={{ color: "#e2e8f0" }}>{t.shares || ""}</TableCell>
                            <TableCell
                              align="right"
                              sx={{ color: t.pnl > 0 ? "#10b981" : t.pnl < 0 ? "#ef4444" : "#e2e8f0", fontWeight: 600 }}
                            >
            ...
            {optimizeResults && !loading && (
              <Paper sx={{ p: 2, bgcolor: "#ffffff", borderRadius: 2, mt: 2 }}>
                <Typography variant="h6" gutterBottom>
                  🔍 Optimization Results ({optimizeResults.combinations_tested} combinations tested)
                </Typography>
                <Typography variant="subtitle1" color="success.main" fontWeight={700} gutterBottom>
                  Best Return: +{optimizeResults.best_return}% | Sharpe: {optimizeResults.best_sharpe}
                </Typography>
                <Typography variant="body2" gutterBottom>
                  Best Parameters: {JSON.stringify(optimizeResults.best_params)}
                </Typography>
                <Divider sx={{ my: 2 }} />
                <Typography variant="subtitle2" gutterBottom>Top 10 Combinations</Typography>
                <TableContainer sx={{ maxHeight: 300 }}>
                  <Table size="small">
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell sx={{ fontWeight: 700 }} align="right">Win Rate</TableCell>
                        <TableCell sx={{ fontWeight: 700 }} align="right">MaxDD</TableCell>
                        <TableCell sx={{ fontWeight: 700 }} align="right">Trades</TableCell>
                      </TableRow>
                    </TableHead>
                    <TableBody>
                      {optimizeResults.top_10.map((r, i) => (
                        <TableRow key={i} sx={i === 0 ? { bgcolor: "#f0fdf4" } : {}}>
                          <TableCell>{i + 1}</TableCell>
                          <TableCell sx={{ fontSize: "0.75rem" }}>
                            {Object.entries(r.params).map(([k, v]) => `${k}=${v}`).join(", ")}
                          </TableCell>
                          <TableCell align="right" sx={{ color: r.total_return_pct > 0 ? "#10b981" : "#ef4444", fontWeight: 700 }}>
                          ...
                          <TableCell align="right">-{r.max_drawdown_pct}%</TableCell>
