...
const InstitutionalView = () => {
    ...
    return (
      <Box>
        <Typography color="text.secondary">
          No institutional holding data yet. Run `holdings_quarterly` task or wait for next scheduled fetch.
        </Typography>
        {stock && (
          <Box mt={2}>
            <Typography variant="body2">
              From Yahoo: {stock.institution_count} institutions, top 10 own {((stock.top_ten_institution_ownership || 0) * 100).toFixed(1)}%
            </Typography>
          </Box>
        )}
      </Box>
  ...
  return (
    <Box>
      ...
      {pieOption && (
        <Paper sx={{ p: 2, mb: 2 }}>
          <Typography variant="h6" gutterBottom>Top Holders</Typography>
          <ReactEChartsCore echarts={echarts} option={pieOption} style={{ height: 350 }} />
        </Paper>
      )}
