...
const MacroOverviewView = () => {
  const { data: series, isLoading } = useResource("macro-series", "/macro-series/");
  const theme = useChartTheme();

  if (isLoading) return <ScaleLoader loading />;
  if (!series || !Array.isArray(series) || series.length === 0) {
    return <Typography color="text.secondary">No macro data. Run: make fetch-fred</Typography>;
  }
...
const MacroCard = ({ series, theme }) => {
  ...
  const option = recent.length > 1 ? {
    grid: { top: 5, bottom: 20, left: 40, right: 10 },
    xAxis: { type: "category", data: recent.map((p) => p.date), show: false },
    ...
    <Card>
      <CardContent sx={{ py: 1, "&:last-child": { pb: 1 } }}>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Typography variant="body2" fontWeight="bold">{series.title}</Typography>
          {latest && (
            <Box textAlign="right">
              <Typography variant="body1" fontWeight={600}>
                {latest.value.toFixed(2)}{series.units ? ` ${series.units}` : ""}
              </Typography>
              {delta != null && (
                <Typography
                  variant="caption"
                ...
                >
                  {delta > 0 ? "↑" : delta < 0 ? "↓" : "→"} {Math.abs(delta).toFixed(2)} vs 7d ago
                </Typography>
              )}
            </Box>
          )}
