  ...
  return (
    <Box p={3}>
      ...
      <Grid container spacing={2}>
          ...
          <Grid item xs={12}>
            <Paper
              sx={{
                p: 2,
                borderRadius: 2,
                bgcolor: portfolio.total_pnl >= 0 ? "#064e3b" : "#7f1d1d",
              }}
            >
              ...
              <List dense disablePadding>
                {oversold.map((s) => (
                  <ListItem
                    ...
                    onClick={() =>
                      navigate(`/stocks/${s.stock__id}/historical/price`)
                    }
                  >
                    <ListItemText
        ...
        <Grid item xs={12} sm={6} md={4}>
          <Section
            icon={<NotificationsActiveIcon sx={{ color: AMBER }} />}
            title="Triggered Alerts"
            color={AMBER}
            empty="No unread alerts"
          >
            {alerts?.length > 0 && (
              <List dense disablePadding>
                {alerts.map((a) => (
                ...
                {earnings_this_week.map((e, i) => (
                  <ListItem key={i} disablePadding sx={{ py: 0.3 }}>
                    <ListItemText
                      ...
                      secondary={`${e.report_date} · est EPS $${
                        e.estimated_eps || "—"
                      }`}
                    />
                  </ListItem>
                ...
                {recent_insider_trades.map((t, i) => (
                  <ListItem key={i} disablePadding sx={{ py: 0.3 }}>
                    <ListItemText
                      ...
                      secondary={`${t.insider_name?.split(" ")[0]} · ${
                        t.shares
                      } shares`}
                    />
                  </ListItem>
